...
- (NSInteger)xy_noDataViewMessageLineNum;   //  使用默认占位图, 提供显示文字的行数，可不提供，默认为1
...
- (void)xy_havingData:(BOOL)havingData {
    ...
    //  获取文字行数
    if ([self.delegate    respondsToSelector:@selector(xy_noDataViewMessageLineNum)]) {
        lineNum = (NSInteger)[self.delegate performSelector:@selector(xy_noDataViewMessageLineNum)];
    }
    //  获取颜色
    ...
    //  创建占位图
    self.backgroundView = [self xy_defaultNoDataViewWithImage  :img message:msg messageLine:lineNum color:color offsetY:offset];
}

/**
 默认的占位图
 */
- (UIView *)xy_defaultNoDataViewWithImage:(UIImage *)image message:(NSString *)message messageLine:(NSInteger)lineNum color:(UIColor *)color offsetY:(CGFloat)offset {
    ...
    label.frame          = CGRectMake(0, CGRectGetMaxY(imgView.frame) + 24, sW, label.font.lineHeight * lineNum);
    //  视图
